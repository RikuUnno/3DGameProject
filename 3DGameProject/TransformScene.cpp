...
void TransformScene::Start()
{
    ...
    _parent = SpawnTransformObject_({ {"px","0.0"},{"py","1.2"},{"pz","0.0"}, {"radius","0.55"},{"axis","1.5"}, {"color",std::to_string(GetColor(255,255,255))} });
    _child  = SpawnTransformObject_({ {"px",std::to_string(_childLocalOffset.x)},{"py",std::to_string(_childLocalOffset.y)},{"pz",std::to_string(_childLocalOffset.z)}, {"radius","0.38"},{"axis","1.0"}, {"color",std::to_string(GetColor(255,220,120))} });
    _worldMarker = SpawnTransformObject_({ {"px","0.0"},{"py","1.2"},{"pz","0.0"}, {"radius","0.25"},{"axis","0.8"}, {"color",std::to_string(GetColor(120,220,255))} });
...
void TransformScene::Update(float dtSec)
{
    ...
    if (key.IsKeyInputTrigger(KEY_INPUT_F) && _child) {
...
void TransformScene::Draw()
{
    ...
    DrawString(10, 10, "TransformScene - F:着脱切替 R:リセット ESC:メニュー", GetColor(255, 255, 255));
...
void TransformScene::UpdateParentMotion_()
{
    if (!_parent) return;
    _parent->transform.SetLocalPosition(VGet(std::sin(_driveTime * 0.6f) * 1.2f, 1.2f, std::cos(_driveTime * 0.4f) * 0.8f));
    _parent->transform.SetLocalEulerRad(VGet(0.0f, _driveTime * 1.1f, 0.0f));
}

TransformDebugClass* TransformScene::SpawnTransformObject_(const VariantMap& params)
{
    return dynamic_cast<TransformDebugClass*>(ObjectManager::Instance().Spawn(TransformDebugClass::StaticPoolKey(), params));
}

void TransformScene::DrawGridFloor_(float y, int halfCells, float step)
{
